...
def prep_model2D():
  ...
  model.add(Conv2D(16, kernel_size=8, strides=2, activation='relu', input_shape=(128, 128, 1)))
  model.add(Dropout(0.1))
  model.add(Conv2D(32, kernel_size=5, strides=2, activation='relu'))
  model.add(Dropout(0.1))
...
for ii, split in enumerate(np.arange(ntimes)): 
 for sslice in np.arange(0, 128, 10): 
  ...
  ## 
  model   = prep_model2D()  ##  prep_model2D()
